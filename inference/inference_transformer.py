...
class UNet3D:
    ...
    def get_model_8(self):
        ...
        # fmnet07-3MHSA-Res4     loss=0.10323 val_vt_dice=0.8452 val_an_dice=0.9097 param=1,627,071
        attention1 = MultiHeadAttention(4, key_dim=3, dropout=0)(add4, add4)
        att_add1 = Add()([attention1, add4])
        attention2 = MultiHeadAttention(4, key_dim=3, dropout=0)(att_add1, att_add1)
        att_add2 = Add()([attention2, att_add1])
        attention3 = MultiHeadAttention(4, key_dim=3, dropout=0)(att_add2, att_add2)
        attention = Add()([attention3, att_add2])
...
if __name__ == '__main__':
    ...
    weight_name ='../weights/fmnet84.hdf5'
    ...
    for i in range(0, len(filename)):
        ...
        twice, aa, bb, cc, patch_size, image, x_bias, y_bias, z_bias, x_bias2, y_bias2, z_bias2, all_patch, all_patch2, x_bias_add, y_bias_add, z_bias_add, sup_x, sup_y, sup_z = preprocess(
            name=name)
        unet3d = UNet3D(inputshape=(64, 64, 64, 1), classes=3)
        domeResult1, vesselResult = unet3d.test(all_patch, weight_name=weight_name)
        if twice:
            domeResult2 = unet3d.test_twice(all_patch2, weight_name=weight_name)
        else:
            domeResult2 = None
    ...
    for i in range(0, len(saveinfos)):
        savename = saveinfos[i]
        print(savename)
        inference4(i, weight_name=weight_name)
        gc.collect()
        K.clear_session()
    # generate container
    path = '../data/step1/'
